...
const TEST = process.env.MTN_APM_TEST;
...
function Span(transaction, ...args) {
  ...
  if (opts.timer) {
    // before 4.x this will be passed in. it was never publicaly docuemented.
    delete opts.timer;
  }
  if (!opts.childOf) {
...
Span.prototype.setDestinationContext = function (destCtx) {
  process.emitWarning(
    ...
    'DeprecationWarning',
    'MTN_APM_SET_DESTINATION_CONTEXT',
  );
...
function filterCallSite(callsite) {
  ...
  return filename
    ? filename.indexOf('/node_modules/mtn-apm-agent/') === -1
    : true;
}
