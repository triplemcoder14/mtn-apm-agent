...
Client.prototype.config = function (opts) {
  ...
  if (!this._conf.endpoint) this._conf.endpoint = 'http://127.0.0.1:8200';
  ...
  // processed values
  this._conf.endpoint = new URL(this._conf.endpoint);
  ...
  let AgentKeepAlive;
  switch (this._conf.endpoint.protocol) {
    case 'http:':
    ...
    case 'https:':
      ...
      break;
    default:
      throw new Error('Unknown protocol ' + this._conf.endpoint.protocol);
  }


  if (!this._agent || (opts && 'endpoint' in opts)) {
    if (this._agent) {
      this._agent.destroy();
    }
  ...
  const req = this._transportGet(opts, (res) => {
    ...
    streamToBuffer(res, (err, buf) => {
      ...
      if (res.statusCode === 200) {
        // 200: New config available (or no config for the given service.name 
...
function getBasicRequestOptions(method, defaultPath, headers, opts, agent) {
  return {
    ...
    method,
    path:
      opts.endpoint.pathname === '/'
        ? defaultPath
        : opts.endpoint.pathname + defaultPath,
    headers,
  };
}
